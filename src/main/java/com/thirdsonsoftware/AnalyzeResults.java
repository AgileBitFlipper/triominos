 ...
 *   each game.
 *
 *   Futures:  This action should be replaced by the use of the ELK
...
public class AnalyzeResults {
    ...
    public String gamesWonByAPlayer() {
        ...
        while ( it.hasNext() ) {
            ...
            strGamesWonByPlayer.append(String.format("  Player '%s' has won %d times.\n", player.getName(), me.getValue()));
        }
    ...
    public String tilesPlayed() {
        ...
        while ( it.hasNext() ) {
            ...
            strTilesPlayed.append(String.format("  Tile '%s' (%2d) has been played %s times.\n", me.getKey(), tile.getId(), me.getValue()));
        }
    ...
    public String startingTilePlayed() {
        ...
        while ( it.hasNext() ) {
            Map.Entry me = (Map.Entry)it.next();
            Tile tile = (Tile)me.getKey();
            strTilesPlayed.append(String.format("  Tile '%s' (%2d) has been played as the starting tile %s times.\n", me.getKey(), tile.getId(), me.getValue()));
        }
        return strTilesPlayed.toString();
    }
}
